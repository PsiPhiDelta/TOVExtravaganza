...
###############################################################################
# 4) Function to process one file: reads EOS, solves TOV for multiple central Ps,
#    writes a star CSV, and plots M(R).
###############################################################################
def process_eos_file(infile, out_folder="MR"):
    ...
    with open(out_csv, "w", encoding="utf-8") as fcsv:
        ...
        for p_c in p_c_values:
            ...
            # Write a single line: p_c, R, M, e(pc), ...
            row_data = [p_c, R, M] + extras_at_pc
            row_str = ",".join(f"{x:.6e}" for x in row_data)
            fcsv.write(row_str + "\n")

            print(f"File={base_name}, p_c={p_c:.3e} => R={R:.4f}, M={M:.4f}")
    ...
    print(f"Done with {base_name}: Wrote {len(p_c_values)} lines to '{out_csv}' and plot to '{out_pdf}'\n")

###############################################################################
# 5) MAIN => Ask user for a folder, process each CSV.
###############################################################################
def main():
    # Let user pick a folder
    folder_in = input("Enter the folder containing your EOS CSV files: ").strip()
    if not os.path.isdir(folder_in):
        print(f"'{folder_in}' is not a valid directory! Bailing out.")
        return
    ...
    print("\nOh boy oh boy! We will process the following files:")
    for fn in files:
        print("  ", fn)

    # Output folder (hard-coded as "MR" here)
    out_folder = "MR"
    print(f"\nWe'll write star solutions and plots into the '{out_folder}/' folder.\n")
    ...
    print(f"\nAll done! We processed {count_files} file(s). Enjoy your TOV solutions!\n")
